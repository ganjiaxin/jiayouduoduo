...
import com.hyk.code.common.security.shiro.session.SessionDAO;
import com.hyk.code.common.service.BaseService;
import com.hyk.code.common.service.ServiceException;
...
import com.hyk.code.modules.sys.security.SystemAuthorizingRealm;
import com.hyk.code.modules.sys.utils.LogUtils;
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	...
	public List<Menu> findAllMenuLeft(){
		/*// 清除用户菜单缓存
		UserUtils.removeCache(UserUtils.CACHE_MENU_LIST_PER);
...
//		systemRealm.clearAllCachedAuthorizationInfo();
		...
		return this.getSubMenuTree();
	}
	public List<Menu> getSubMenuTree(){
		List<Menu> menuTree = (List<Menu>) UserUtils.getCache(UserUtils.CACHE_MENU_LIST);
		if(menuTree==null){
			User user = UserUtils.getUser();
//			if (user.isAdmin()){
//				menuTree =getSubMenuTree(new Menu());
//			}else{
				Menu m = new Menu();
				m.setUserId(user.getId());
				m.setId("1");
				menuTree =getSubMenuTree(m);
			//}
			UserUtils.putCache(UserUtils.CACHE_MENU_LIST, menuTree);
		}		
		return menuTree;
	}
	 ...
	 * 获取菜单目录树
	 */
	public List<Menu> getSubMenuTree(Menu menuNew) {
	...
	private List<Menu> getAllSubMenuList(Menu menu) {
		List<Menu> curSubMenu = this.menuDao.findChildByParent(menu);
		
		return curSubMenu;
	}
	
	private Boolean isChildMenu(Menu curMenu) {
		List<Menu> curSubMenu = this.menuDao.findChildByParent(curMenu);
