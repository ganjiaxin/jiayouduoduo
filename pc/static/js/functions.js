...
jQuery(function($) {
  ...
  $.ajax({
    type: "GET",
    url: "https://www.huiucard.com/hykweb/find/allNews",
    // url: "http://localhost:80/param",
    headers: {
    ...
    success: function(data) {
      for (var i = 0; i < 3; i++) {
        var html =
          ...
          '<a href="detailsPage.html?id=' +
          data.list[i].id +
          '" style="position:absolute;z-index:999;width:100%;height:100%;display:block;top:0;left:0;"></a>' +
          '<div class="image bottom20">' +
          '<img src="' +
          data.list[i].ico +
          '" alt="Courses" class="img-responsive border_radius">' +
          "</div>" +
          '<h3 class="bottom18">' +
          data.list[i].title +
          "</h3>" +
          '<p class="bottom17">' +
          escape2Html(data.list[i].content)
            .replace(/<[^<>]+?>/g, "")
            .substring(0, 35) +
          "...</p>" +
          '<div class="bottom16">' +
          data.list[i].createDateStr +
          "</div>" +
          "</div>";
        $("#course_slider").append(html);
      }
