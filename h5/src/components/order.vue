  ...
  <div class="content_panel orderPanel">
    <div class="tab">
      <ul class="oli">
        ...
        <li :class="{active:isCheck===0}" @click="handleCheck(0)">执行中</li>
        <li :class="{active:isCheck===1}" @click="handleCheck(1)">已完成</li>
        <div class="clear"></div>
      </ul>
    </div>
    <cube-scroll
        ...
        <div class="order_box" v-for="(data,index) in items" :key="index">
          <router-link
            ...
            <h4>
              ...
              <span class="error" v-else-if="data.orderStatus==='6'">已退款</span>
              <span class="error" v-else-if="data.orderStatus==='5'">已暂停</span>
  ...
  export default {
    ...
    methods: {
      ...
      loadPage (pageNum, type) {
        let _this = this
        postRequest(this.HOST + '/mine/allOrder', {
          token: _this.token,
          oilStatus: _this.isCheck,
          currPage: pageNum
