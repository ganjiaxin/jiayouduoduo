...
public class HykAddressService extends CrudService<HykAddressDao, HykAddress> {
	 ...
	 * @return
	 */
	public HykAddress getDefaultAddressByUserId(String userId){
		return hykAddressDao.getDefaultAddressByUserId(userId);
	};


	public HykAddress getByHykAddress(HykAddress hykAddress){
		return hykAddressDao.getByHykAddress(hykAddress);
	};//获取该对象
}