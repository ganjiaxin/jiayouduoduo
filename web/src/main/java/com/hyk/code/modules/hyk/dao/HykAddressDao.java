...
public interface HykAddressDao extends CrudDao<HykAddress> {
     ...
     * 删除地址
     */
    @Update("update hyk_address set del_flag=1 where id=#{addressId}")
     ...
     * @return
     */
    HykAddress getDefaultAddressByUserId(String userId);

    HykAddress getByHykAddress(HykAddress hykAddress);//获取该对象
}