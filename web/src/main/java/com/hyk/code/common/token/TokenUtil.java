...
public class TokenUtil {
    ...
    private static HykUserDao hykUserDao = SpringContextHolder.getBean(HykUserDao.class);
                ...
                if(claims!=null){
                    ...
                    if(expdate>nowMillis){//判断token有效性
                       /* String username=(String)claims.get("username");
                        ...
                        hykUser=hykUserDao.getByPhone(hykUser);
                        if(username.equals(hykUser.getPhone())&&password.equals(hykUser.getPassword())){
     ...
     * @return
     */
    public static HykUser getAppCurrentUser(String jwt){
        String signingKey =PropertiesUtil.getValue("signingKey");//此处就是在服务器定义的自己的密匙
        HykUser hykUser=new HykUser();
        String params="";
            ...
            if(header.equals(headerNew) && sign.equals(signNew)){//进行安全校验（头部和签名）
                ...
                if(claims!=null){
                    ...
                    if(expdate>nowMillis){//判断token有效性
                        ...
                        HykUser current=hykUserDao.get(userId);

                        if(username.equals(current.getPhone())&&password.equals(current.getPassword())){
                            params="success";//校验成功，有此用户
                            hykUser=current;
                        }else{
                            params="用户名或密码错误---faile";
            ...
            if(header.equals(headerNew) && sign.equals(signNew)){//进行安全校验（头部和签名）
                ...
                if(claims!=null){
                    ...
                    if(expdate>nowMillis){//判断token有效性
                        ...
                        if(username.equals(current.getLoginName())&&password.equals(current.getPassword())){
                            params="success";//校验成功，有此用户
                            user=current;
                        }else{
                            params="用户名或密码错误---faile";
