...
mSift.prototype = {
    ...
    Select: function() {
        ...
        if (_this.ReData.length > 0) {
			　var str = _this.ReData[_this.SelIndex].replace(/\*/g, '*').replace(/\|/g, '|');
			 var name = str.split(',')[0];
			 var id =str.split(',')[1];
            _this.Target.value = name;
            console.log(_this.Target.id)
	         $('#'+_this.Target.id).prevAll('input').val(id);
            _this.Clear();
        }
        setTimeout(function() {
            _this.Target.focus();
    ...
    Get: function() {
        ...
        for (var i = 0; i < _this.Data.length; i++) {
		    ...
		    var id =_this.Data[i].id;
            if (name.toLowerCase().indexOf(_this.Target.value.toLowerCase()) >= 0) {
                _this.ReData.push(name+','+id);
        ...
        for (var i = 0; i < _this.ReData.length; i++) {
			if(_this.Target.value.indexOf(' ') >= 0){
				...
				cResult += '<li style="padding:0 5px;line-height:30px;cursor:default;" onmouseover="' + _this.oo + '.ChangeOn(this);' + _this.oo + '.SelIndex=' + i + ';" onmousedown="' + _this.oo + '.Select();">' + _this.ReData[i].split(',')[0].replace(cRegEx,
				function(s) {
					return '<span style="font-weight:bold;font-style:normal;">' + s + '</span>';
				}); + '</li>';
        ...
        if (cResult =='') {
            ...
            _this.TgList.style.cssText = 'display:block;z-index:99999;position:absolute;background:#fff;border:#C9C9C9 solid 1px;margin:-1px 0 0;padding: 5px;list-style:none;font-size:12px;';
            _this.TgList.style.top = $(_this.Target).innerHeight()+2+'px';
            _this.TgList.style.left = 0;
            _this.TgList.style.width = $(_this.Target).innerWidth()+2 + 'px';
        }
        var oLi = _this.TgList.getElementsByTagName('li');
        if (oLi.length > 0) {
            oLi[_this.SelIndex].style.cssText = 'background:#5FB878;padding:0 5px;line-height:30px;cursor:default;color:#fff;';
        }
    },
    ChangeOn: function(oObj) {
        ...
        oObj.style.cssText = 'background:#5FB878;padding:0 5px;line-height:30px;cursor:default;color:#fff;';
    },
    Clear: function() {
        var _this = this;
        if (_this.TgList) {
