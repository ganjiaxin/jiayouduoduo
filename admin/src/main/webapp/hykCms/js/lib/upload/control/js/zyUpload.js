	...
	$.fn.zyUpload = function(options,param){
		...
		return this.each(function(){
			...
			var defaults = {
					...
					url              : rootUrl+"/uploadFile/upload",  	// 上传文件的路径fdg
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
				    ...
				    html += '							<img class="upload_image" src="js/lib/upload/control/images/add_img.png" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width)" />';
				    html += '						</div>';
				    html += '					</a>';
				    html += '				</div>';
					html += '			</div>';
			...
			this.createCorePlug = function(){
				var params = {
					...
					onSelect: function(selectFiles, allFiles) {
						...
						self.funSetStatusInfo(selectFiles);  // 显示统计信息
						var html = '', i = 0;
						// 组织预览html
					...
					onSuccess: function(file, response) {
                        ...
                        $(parent.document.forms[0]).find('input[name='+dom+']').val(imgUrl+response.url);
                        $(parent.document.forms[0]).find('.'+dom).html('<img src='+imgUrl+response.url+' style="width:auto;height:125px;">')
                        layer.msg('图片上传成功', {
