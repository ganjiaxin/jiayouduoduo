...
public interface HykGoodsDao extends CrudDao<HykGoods> {
     ...
     * @return
     */
    @Select("SELECT * from hyk_goods where `status`=1 and goods_type=#{type} and del_flag=0 ORDER BY create_date desc")
     ...
     * @return
     */
    @Select("SELECT * from hyk_goods where status =1 and goods_type=#{type} and del_flag=0 order by label asc limit 2")
